...
use egui::{Key, Modifiers, PointerButton, Pos2, Response};
use rand::Rng;
           ...
           for i in -(game_board.brushsize / 2) - 1..game_board.brushsize / 2 {
            for j in -(game_board.brushsize / 2) - 1..game_board.brushsize / 2 {
                if game_board
                    ...
                    .is_some() && game_board.contents.get((((i + (y as i32)) * col_count + (j + x as i32)) as usize)).unwrap_or(&game_board.contents[(( (y as i32)) * col_count + (x as i32)) as usize]).material == VOID
